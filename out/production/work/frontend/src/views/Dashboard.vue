...
<!--  &lt;!&ndash; 전역 로딩 오버레이 (메모 모달과 동일하게 body로 텔레포트) &ndash;&gt;-->
<!--  <Teleport to="body">-->
...
const kpi = computed(() => ({ users: kpiUsers.value, teams: kpiTeams.value }))
...
/* 지점 목록: centers에서 branchId/branchName으로 유추 */
const branches = computed(() => {
  ...
  for (const c of centers.value || []) {
    ...
    if (!map.has(c.branchId)) {
      map.set(c.branchId, {
        id: c.branchId,
        name: c.branchName || `지점 ${c.branchId}`,
      })
    }
  }
...
async function confirmBranches() {
  ...
  await runBusy(async () => {
    try {
      await Promise.all([loadBranchDbs(ids), loadBranchUsers(ids)])
    } finally {
...
async function confirmCenters() {
  ...
  await runBusy(async () => {
    try {
      await Promise.all([loadCenterDbs(ids), loadCenterUsers(ids)])
    } finally {
      openCenterPicker.value = false
    }
  })
...
/* 집계: 지점별 */
const branchAgg = computed(() => {
  const bs = pickedBranches.value
  if (bs.size === 0)
    return {
      totalUsers: 0,
      dbRangeWithDup: 0,
  ...
  // 선택된 지점 직원 (STAFF + MANAGER)
  const staffInBranches = usersForBranches.value.filter(
      (u) =>
          bs.has(u.branchId) && (u.role === 'STAFF' || u.role === 'MANAGER'),
  )
  const staffIds = new Set(staffInBranches.map((u) => u.id))

  const r = dbsBranches.value.filter(
      (x) => bs.has(x.branchId) && inRange(x.created),
  )
  const a = dbsBranches.value.filter((x) => bs.has(x.branchId))
  return {
    totalUsers: staffIds.size,
    dbRangeWithDup: r.length,
    dbRangeOnly: r.filter((x) => x.type !== '중복').length,
...
const centerAgg = computed(() => {
  ...
  const staffInSelectedCenters = usersForCenters.value.filter(
      (u) => cs.has(u.centerId) && (u.role === 'STAFF' || u.role === 'MANAGER'),
  )
  const staffIds = new Set(staffInSelectedCenters.map((u) => u.id))

  const r = dbsCenters.value.filter(
      (x) => cs.has(x.centerId) && inRange(x.created),
  )
  const a = dbsCenters.value.filter((x) => cs.has(x.centerId))
  return {
    totalUsers: staffIds.size,
...
const userAgg = computed(() => {
  ...
  const r = dbsUser.value.filter(
      (x) => x.userId === id && inRange(x.created),
  )
  const a = dbsUser.value.filter((x) => x.userId === id)
  return {
    totalUsers: 1,
...
const normDbs = (raw = []) =>
    raw.map((r) => ({
      userId: r.userId ?? r.user_id, // 담당자 ID
      centerId: r.centerId ?? r.center_id, // 센터/팀 ID (JOIN으로 내려줌)
      branchId: r.branchId ?? r.branch_id ?? null, // 지점 ID (JOIN으로 내려줌)
      created:
          (r.created ??
              r.customer_created_at ??
              r.duplicate_created_at ??
              ''
          ).slice(0, 10),
      // type: '중복' | '유효/최초' 등 — 서버에서 isDuplicate 또는 division으로 내려줌
      type: r.type ?? (r.isDuplicate ? '중복' : r.customer_division ?? '유효'),
...
async function loadBranchDbs(branchIds = []) {
  ...
  dbsBranches.value = normDbs(res.data)
}
